...
def main():
    ...
    if int(get_git_status()) > 0:
        ...
        os.chdir("ServiceRentableItemsPlugin")

        current_number_of_commits = get_commit_count()
        packages = [
            'Microting.eForm', 'Microting.eFormApi.BasePn', 'Microting.eFormBaseCustomerBase', 'Microting.eFormRentableItemBase'
        ]
        process_repository('ServiceRentableItemsPlugin.csproj', packages, 'eform-service-rentableitem-plugin')
        ...
        if new_number_of_commits > current_number_of_commits:
            ...
            os.chdir("..")
            run_command(f"github_changelog_generator -u microting -p eform-service-rentableitem-plugin -t {os.getenv('CHANGELOG_GITHUB_TOKEN')}", shell=True)
